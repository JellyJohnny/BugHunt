...
public class StrategyState : BaseState
{
    public override void EnterState(ModeManager c)
    {
        ...
        c.currentAgent.GetComponent<NavMeshAgent>().enabled = true;
        c.currentAgent.GetComponent<Player>().fpMove.enabled = false;
        c.currentAgent.GetComponent<Player>().mLook.enabled = false;
